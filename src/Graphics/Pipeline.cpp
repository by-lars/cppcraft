...
namespace ZuneCraft {
	...
	Pipeline& Pipeline::AddShader(const std::string& name) {
	...
	void Pipeline::PushCommand(int count) {
		ZC_ASSERT(m_CommandQueue != nullptr, "Command can only be pushed after build");
		RenderCommand cmd{ count, 0, 0, 0 };		
		m_CommandQueue->Push(cmd);
	}
	...
	Ref<RenderTarget> Pipeline::GetRenderTarget() {
