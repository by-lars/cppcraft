...
namespace ZuneCraft {
	...
	static GLenum DrawModeToNative(DrawMode drawMode) {
		switch (drawMode) {
	...
	Ref<CommandQueue> OpenGLAPI::CreateCommandQueue(DrawMode drawMode, UsageHint hint) {
		GLenum glDrawMode = DrawModeToNative(drawMode);
	...
	void OpenGLAPI::DrawArrays(DrawMode mode, int count) {
		GLenum drawMode = DrawModeToNative(mode);
