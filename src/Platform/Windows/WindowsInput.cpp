...
namespace ZuneCraft {
	...
	bool Input::GetActionJump() {
		if (!IsFocused) return false;
		return glfwGetKey((GLFWwindow*)Game::Get().GetWindow().GetNativeWindow(), GLFW_KEY_SPACE) == GLFW_PRESS;
	}
	...
	glm::vec2 Input::GetRotationAxies() {
		...
		glm::vec2 offsets((x - lastX) * 0.5f, (y - lastY) * 0.5f);
